class Gameboard {
  ...
  gameover: boolean;
  won: boolean;
  drawProbabilityMap: boolean;
  mineProbabilityMap: Map<number, number>;
  highlightedX: number;
  highlightedY: number;
  firstActionSave: boolean;
  ...
  createCellsSave(x: number, y: number) {
    let hasMine = Array(this.mineCount).fill(true).concat(
  ...
  createCellsFromArray(hasMine: boolean[]) {
  ...
  iterateNeighbours(x: number, y: number, callback: (neighbour: Cell, neighbourX?: number, neighbourY?: number) => void) {
    for (let dx of [-1, 0, 1]) {
      for (let dy of [-1, 0, 1]) {
  ...
  // returns true, if mine was revealed
  revealCell(x: number, y: number, revealedCells: Cell[]) {
    let cell = this.board[x][y];
  ...
  getCoordinates(canvasX: number, canvasY: number): [number, number] {
    return [Math.floor(canvasX / this.cellSize), Math.floor(canvasY / this.cellSize)];
  }

  validCoordinates(x: number, y: number) {
